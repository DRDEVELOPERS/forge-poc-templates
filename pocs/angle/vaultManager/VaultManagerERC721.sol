...
abstract contract VaultManagerERC721 is
    ...
    VaultManagerStorage
{
    ...
    function tokenURI(uint256 vaultID) external view returns (string memory) {
        ...
        return
            bytes(_baseURI).length != 0
                ? string(abi.encodePacked(_baseURI, string(buffer)))
                : "";
    }
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, vaultID);
        if (!_checkOnERC721Received(from, to, vaultID, _data))
    ...
    function _isApprovedOrOwner(
        ...
        return (spender == owner ||
            _getApproved(vaultID) == spender ||
            _operatorApprovals[owner][spender] == 1);
    }
    ...
    function _mint(address to) internal returns (uint256 vaultID) {
        ...
        if (!_checkOnERC721Received(address(0), to, vaultID, ""))
            revert NonERC721Receiver();
    }
